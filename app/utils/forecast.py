...
def in_sample_prediction(
    ...
    ─ exog_hist : DataFrame déjà aligné sur la grille custom (colonnes brutes).
    """
    # mêmes features, même ordre que pendant l’entraînement
    X_pca = pca.transform(
    ...
    return pd.Series(y_pred_real, index=exog_hist.index, name="y_hat")
...
def auto_update_model_with_latest_data(cible, model, scaler_exog, scaler_target, pca):
    ...
    new_endog_series = pd.Series(new_endog_norm, 
